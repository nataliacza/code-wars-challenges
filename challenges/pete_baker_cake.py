...
def cakes(recipe: dict, available: dict) -> int:
    # Variable that contains the minimum full cakes
    minimum = float("inf")

    for k, v in recipe.items():
        if k in available.keys():                       # check if available ingredients are in recipe - if not, we know we cannot make any cake
            if available[k] // recipe[k] < minimum:     # check the minimum of what we can get out of each ingredient
                minimum = available[k] // recipe[k]     # replace the value of minimum
        else:
            return 0
